		...
		if self.sample == 'random':
			...
			for indices in indices_list:  # 遍历每一个采样序列长度
				...
				for index in indices:  # 遍历每个采样序列中的每一张图片
					...
					of_path = self.getOfPath(img_path)
					imgrgb = Image.open(img_path)
					imgof = Image.open(of_path)
					imgseq.append(imgrgb)
					flowseq.append(imgof)
				seq = [imgseq, flowseq]
				if self.transform is not None:
					seq = self.transform(seq)  # torch.Size([16, 5, 128, 64])
				imgs_list.append(seq)
			imgs_list = tuple(imgs_list)
			imgs_array = torch.stack(imgs_list, 0)  # 获得整个行人序列的密集采样序列 torch.Size([k, 16, 5, 128, 64])
